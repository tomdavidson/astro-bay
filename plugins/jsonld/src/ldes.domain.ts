...
const hashContent = (content: string): string => {
  const hash = content.length === 0 ?
    0 :
    Array.from({ length: content.length }).reduce((accumulator: number, _unused: unknown, index: number) =>
      Math.imul(HASH_PRIME, accumulator) + content.charCodeAt(index), 0)

  return Math.abs(hash).toString(RADIX)
}

export const buildState = (routes: ReadonlyArray<RouteJsonLd>): LdesState =>
  new Map(routes.map(route => [route.route, hashContent(JSON.stringify(route.node))] as const))
...
const detectCreates = (
  ...
  [...current.entries()].filter(([route]) => !previous.has(route)).map(([route]) => ({
...
const detectUpdates = (
  ...
  [...current.entries()].filter(([route, hash]) => {
...
const detectDeletes = (
  ...
  [...previous.keys()].filter(route => !current.has(route)).map(route => ({
...
export const serializeChangeFeed = (
  ...
  JSON.stringify(
    {
      '@context': { ...context, as: 'https://www.w3.org/ns/activitystreams#' },
...
const isStringRecord = (value: unknown): value is Record<string, string> =>
  ...
  value !== null &&
  !Array.isArray(value) &&
  objectEntries(value).every(([key, entryValue]) => typeof key === 'string' && typeof entryValue === 'string')

const parseUnknownJson = (raw: string): Result<unknown, LdesParseError> =>
  Result.fromThrowable(
    () => JSON.parse(raw) as unknown,
    () => ({ type: 'LdesStateCorrupt' as const, path: '' }),
  )()

const parseRecord = (raw: string): Result<Record<string, string>, LdesParseError> =>
  parseUnknownJson(raw).andThen(parsed =>
    isStringRecord(parsed) ?
      ok<Record<string, string>, LdesParseError>(parsed) :
      err({ type: 'LdesStateCorrupt' as const, path: '' })
  )

export const parseState = (raw: string): Result<LdesState, LdesParseError> =>
