...
import { contentHub, getTransforms, registerTransforms, resetTransformRegistry } from './integration.ts'
...
describe('contentHub', () => {
  ...
  test('contentHub_setsExpectedIntegrationName', () => {
    const integration = contentHub({ name: 'writing', collections: ['vault'] })
  ...
  test('astro:config:setup_duplicateHubName_throws', async () => {
    ...
    await firstHooks['astro:config:setup']!(makeConfigSetupContext({ isRestart: false }).params)

    await expect(secondHooks['astro:config:setup']!(makeConfigSetupContext({ isRestart: true }).params))
  ...
  test('astro:config:done_injectsTypes', async () => {
    const integration = contentHub({ name: 'writing', collections: ['vault'], transforms: [noopTransform] })
    ...
    expect(injectTypes).toHaveBeenCalledWith(
      expect.objectContaining({ filename: 'content-hub-writing.d.ts' }),
    )
  })
  ...
  test('astro:build:done_logsBuildComplete', async () => {
    const integration = contentHub({ name: 'writing', collections: ['vault'], jsonld: { enabled: false } })
  ...
  test('contentHub_invalidConfig_throwsWithFieldSpecificMessage', () => {
    expect(() => contentHub({ name: 'broken', collections: [] })).toThrow('config error')

    expect(() => contentHub({ name: 'broken', collections: [] })).toThrow('collections')
  })

  test('astro:config:setup_deprecatedPagination_logsWarning', async () => {
    const integration = contentHub({ name: 'legacy', collections: ['vault'], pagination: { pageSize: 15 } })
  ...
  test('astro:config:setup_browseSet_noDeprecationWarning', async () => {
    const integration = contentHub({ name: 'modern', collections: ['vault'], browse: { pageSize: 30 } })
