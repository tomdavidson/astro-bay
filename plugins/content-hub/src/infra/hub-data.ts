...
import type { HubData, NormalizedEntry, TransformContext, TransformWarning } from '../types.ts'
...
export type HubConfig = Omit<ResolvedConfig, 'transforms'> & {
...
const aggregateHubEntries = (
  ...
  aggregateEntries(config.collections, getCollection, {
    topics: config.taxonomy.field,
    feedCategory: config.taxonomy.feedCategoryField,
  })
...
const warnOnUidFallbacks = (
  ...
  entries.filter(item => item.uidFallback).forEach(({ entry }) => {
    logger.warn(
...
const warnOnTransformErrors = (warnings: ReadonlyArray<TransformWarning>, logger: Logger): void => {
  warnings.forEach(warning => {
    logger.warn(
      `astro-content-hub: transform error for entry "${warning.uid}": ${
        String(warning.cause)
...
const getAllTransforms = async (
  ...
  return ancestorTransform === undefined ? combined : [...combined, ancestorTransform]
}
...
const transformEntries = async (
  ...
  const ctx: TransformContext = { allEntries: raw, siteUrl: config.siteUrl, cache: new Map() }
...
const markDraftPreview = (entry: NormalizedEntry): NormalizedEntry =>
  entry.draft ? { ...entry, meta: { ...entry.meta, isDraftPreview: true } } : entry
...
const selectPublishedEntries = (
  ...
  return includeDrafts ? transformed.map(markDraftPreview) : filterPublished(transformed)
}

const assertNoUidCollisions = (config: HubConfig, published: ReadonlyArray<NormalizedEntry>): void => {
  const collisionResult = detectCollisions(published)
  if (collisionResult.isOk()) return

  match(collisionResult.error).with({ type: 'UidCollision' }, error => {
