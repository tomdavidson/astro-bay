...
export { aggregateEntries, filterPublished, normalizeEntry, sortByDate } from './aggregate.ts'

export { buildTopicMap, entriesForTopic, groupByTopic, slugifyTopic, topicsWithCounts } from './taxonomy.ts'

export { collectAliasRoutes, detectCollisions } from './permalinks.ts'
...
import type { EntryTransform, NormalizedEntry, TransformBatchResult, TransformContext } from './types.ts'
 ...
 */
export const toRssItem = (entry: NormalizedEntry, articleBase: string): RssItem => ({
  title: entry.title,
  pubDate: entry.date,
...
export const toFeedItems = (
  ...
  entries.filter(e => !e.draft && e.source !== 'feed').map(e => toRssItem(e, articleBase))
...
export const runTransformsDetailed = (
  ...
  transforms: ReadonlyArray<EntryTransform>,
  ctx: TransformContext,
): Promise<TransformBatchResult> => runTransformsInternal(entries, transforms, ctx)
